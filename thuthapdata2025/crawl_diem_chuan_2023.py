...
# Đường dẫn file 2025 đã làm trước đó để lấy kho mã trường
CSV_2025_PATH = r"d:\Dev\Code\admission-rag-system\thuthapdata2025\diem_chuan_2025.csv"

def get_unique_school_codes_from_2025():
    """
    Lấy danh sách mã trường duy nhất từ diem_chuan_2025.csv để làm mốc truy vấn
    """
    codes = set()
    try:
...
def fetch_school_and_scrape(school_code):
    """
    Tìm ID -> Lấy danh sách điểm chuẩn 2023 -> Trả về mảng records kết hợp Retry
    """
    ...
    # URL lấy điểm năm 2023
    url = f"https://diemthi.vnexpress.net/tra-cuu-dai-hoc/loadbenchmark/id/{s_id}/year/2023/sortby/1/block_name/all"
...
def main():
    ...
    print(f"Tìm thấy kho {len(codes)} Mã trường. Bắt đầu tìm kiếm đối chiếu vào VnExpress cho dữ liệu điểm chuẩn năm 2023...\n")
    ...
    output_file = os.path.join(output_dir, "diem_chuan_2023.csv")
    ...
    print(f"\n=> Đã thu hoạch thành công {len(all_records)} dòng điểm chuẩn 2023.")
